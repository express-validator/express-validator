...
export function checkSchema<Req = any, Res = any, NextFn = any>(schema: Schema, defaultLocations: Location[] = validLocations) {
  return Object.keys(schema).map(field => {
    const config = schema[field];
    const chain = check<Req, Res, NextFn>(field, ensureLocations(config, defaultLocations), config.errorMessage);
