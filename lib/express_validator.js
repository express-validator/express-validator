...
var expressValidator = function(req, res, next) {
  ...
  var _getArrayParam = function(param) {
    var value;
  ...
  req.updateParam = function(name, value) {
    ...
    if (Array.isArray(name) && name.length > 0) {
      if (typeof this.params[name[0]] !== "undefined") {
        arrayRefParam = this.params;
      } else if (typeof this.body[name[0]] !== "undefined") {
        arrayRefParam = this.body;
      }

      if (typeof arrayRefParam !== "undefined") {
        for (var index in name) {
          if (index == (name.length - 1)) {
            arrayRefParam[name[index]] = value;
          } else {
            arrayRefParam = arrayRefParam[name[index]];
          }
        }
      }
    } else {
  ...
  req.filter = function(param) {
    ...
    return filter.sanitize(currentValue);
  };
