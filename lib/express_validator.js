...
var expressValidator = function(options) {
  ...
  _.forEach(validator, function(method, methodName) {
    if (methodName.match(/^is/) || _.includes(additionalValidators, methodName)) {
      ValidatorChain.prototype[methodName] = utils.makeValidator(methodName, validator);
    }
  ...
  ValidatorChain.prototype.withMessage = function(message) {
    if (this.lastError) {
      if (this.lastError.isAsync) {
        ...
        var error = utils.formatErrors.call(this.lastError.context, this.lastError.param, message, this.lastError.value);
        ...
        var errorMessage = utils.formatErrors.call(this, this.lastError.param, message, this.lastError.value);
  ...
  utils.mapAndExtend(options.customValidators, ValidatorChain.prototype, utils.makeValidator);
  ...
  return function(req, res, next) {
    ...
    // Extend existing validator. Fixes bug #341
    if (req[validatorChainSymbol] && req[validatorChainSymbol] !== ValidatorChain) {
      ValidatorChain = req[validatorChainSymbol];
      utils.mapAndExtend(options.customValidators, ValidatorChain.prototype, utils.makeValidator);
    }
    req[validatorChainSymbol] = ValidatorChain;
    ...
    req.check = function(param, failMsg) {
      ...
      return new ValidatorChain(param, failMsg, req, utils.locate(req, param), options);
    };
